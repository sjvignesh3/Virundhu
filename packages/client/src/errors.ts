...
import { API_ERROR_CODES, type ApiErrorCode } from "@virundhu/shared";
...
export class ClientApiError extends Error {
  ...
  constructor(code: ApiErrorCode, message: string, status = 400, cause?: unknown) {
    super(message);
 ...
 */
export function fromPostgrest(err: PostgrestError): ClientApiError {
  const code = err.code;
...
export function fromAuth(err: AuthError): ClientApiError {
...
export async function unwrap<T>(
  op: PromiseLike<{ data: T | null; error: PostgrestError | null }>,
): Promise<T> {
