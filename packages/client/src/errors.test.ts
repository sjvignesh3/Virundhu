...
const pg = (code: string, message: string): PostgrestError => ({
  message,
  details: "",
  hint: "",
  code,
...
describe("fromPostgrest", () => {
  ...
  it("maps P0001 'invalid transition' → INVALID_STATUS_TRANSITION", () => {
    const e = fromPostgrest(
      pg("P0001", "illegal transition ACCEPTED -> COMPLETED"),
    );
...
describe("fromAuth", () => {
  const auth = (message: string): AuthError =>
    ({ message, name: "AuthError", status: 400 }) as unknown as AuthError;
