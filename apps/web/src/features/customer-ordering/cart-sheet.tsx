...
export function CartSheet({
  ...
  async function handlePlace() {
    ...
    setError(null);
    try {
      // Prefer the server-side atomic checkout when the repo exposes it
      // (API mode). Falls back to the pure client-side path in local mode
      // so the offline demo still works.
      const order = repos.orders.checkout
        ? await repos.orders.checkout({
      ...
      cart.clear();
      onOpenChange(false);
      // Public confirmation route uses orderNumber so customers can share
      // the link and it works cross-device without needing local storage.
      router.push(`/order/${store.slug}/success/${order.orderNumber}`);
    } catch (err) {
      const msg =
