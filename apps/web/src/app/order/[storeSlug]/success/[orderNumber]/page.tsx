...
import { ApiOrderRepo } from "@/lib/repositories";
import type { Order } from "@/lib/domain/types";

export default function OrderSuccessPage() {
  const params = useParams<{ storeSlug: string; orderNumber: string }>();
  const repos = useRepos();

  const loader = React.useCallback(
    async (r: import("@/lib/repositories").Repos) => {
      // API mode has a dedicated public endpoint for order-by-number.
      if (r.orders instanceof ApiOrderRepo) {
        return r.orders.getPublicByNumber(params.storeSlug, params.orderNumber);
      }
      // Local mode: look up by number within the current store.
      const store = await r.stores.getBySlug(params.storeSlug);
      if (!store) return null;
      return r.orders.getByNumber(store.id, params.orderNumber);
    },
    [params.storeSlug, params.orderNumber],
  );
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-16">
        <EmptyState
          ...
          description="We couldn't find this order. If you just placed it, please try again."
          action={
            <Button asChild>
